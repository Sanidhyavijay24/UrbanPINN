...
export default function SimulationPage() {
  ...
  return (
    <main className="relative w-screen h-[100dvh] overflow-hidden bg-space-950">
      <Scene windData={windData} sliceHeight={sliceHeight} />
      
      {/* Ultra-compact Mobile HUD layout */}
      <div className="absolute top-2 left-2 md:top-6 md:left-6 glass-elevated p-3 md:p-6 w-[calc(100vw-16px)] md:w-80 z-10 flex flex-col gap-1.5 md:gap-6 text-white">
        <div>
          <h1 className="text-[14px] md:text-xl font-bold font-sans tracking-wide">URBAN PINN</h1>
          <p className="text-[10px] md:text-sm text-space-400">Micro-climate CFD Simulator</p>
        </div>
        
        <div className="flex flex-col gap-1.5 md:gap-2">
          <label className="text-[9px] md:text-xs uppercase tracking-widest text-space-300 font-bold">
            Slice Height: <span className="text-velocity-60 text-sm md:text-lg ml-2">{sliceHeight}m</span>
          </label>
            <input 
              type="range" 
              min="0" 
              ...
              className="w-full h-1 md:h-2 bg-space-800 rounded-lg appearance-none cursor-pointer"
            />
            
            {/* Velocity Spectrum Legend */}
            <div className="pt-2 md:pt-5 mt-0.5 md:mt-2 border-t border-space-800/80">
              <div className="flex justify-between items-center text-[8px] md:text-[11px] font-bold text-space-400 uppercase tracking-widest mb-1 md:mb-2">
                <span>Velocity Spectrum</span>
                <span>Peak: {peakGust.toFixed(1)} m/s</span>
              </div>
              <div className="h-1 md:h-2 w-full rounded-full bg-gradient-to-r from-sky-500 via-emerald-400 via-yellow-400 to-red-500 shadow-[0_0_10px_rgba(16,185,129,0.2)]"></div>
              <div className="flex justify-between text-[7px] md:text-[10px] text-space-500 font-mono mt-0.5 md:mt-2">
                <span>0.0</span>
                <span>Local Max</span>
              </div>
            </div>
            ...
            <div className="pt-2 md:pt-4 mt-0.5 md:mt-2">
              <div className="flex justify-between items-center text-[8px] md:text-[11px] font-bold text-space-400 uppercase tracking-widest mb-1 md:mb-2">
                <span>Ambient Thermal</span>
                <span className="text-red-400">Max: {maxTemp !== -Infinity ? maxTemp.toFixed(1) : "0.0"}°</span>
              </div>
              <div className="h-1 md:h-2 w-full rounded-full bg-gradient-to-r from-blue-600 via-purple-500 via-red-500 via-yellow-400 to-white shadow-[0_0_10px_rgba(138,43,226,0.2)]"></div>
              <div className="flex justify-between text-[7px] md:text-[10px] text-space-500 font-mono mt-0.5 md:mt-2">
